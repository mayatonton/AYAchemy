...
// PBRモード選択（RenderToneMapType をパイプラインから渡す：0..6、6=Legacy->PBR統一）
...
void main()
{
    ...
    else
    {
        ...
        // PBRモード（RenderToneMapType==6）の場合、レガシー材質もPBR合成に統一
        if (uToneMapType == 6)
        {
            // レガシー情報からPBRパラメータを推定
            float gloss_legacy        = spec.a;                                   // legacy gloss proxy
            float perceptualRoughness = clamp(1.0 - gloss_legacy, 0.04, 1.0);
            float metallic            = 0.0;                                      // 多くは非金属
            float ao                  = ambocc;
            ...
            vec3 radL = vec3(0);
            float gloss = 1.0 - perceptualRoughness;
            sampleReflectionProbes(irrL, radL, tc, pos.xyz, norm.xyz, gloss, false, amblit_linear);
            adjustIrradiance(irrL, ambocc);
            ...
            color = pbrBaseLight(diffuseColor, specularColor, metallic,
                                 vdir, norm.xyz, perceptualRoughness,
                                 light_dir, sunlit_linear, scol,
                                 radL, irrL,
                                 vec3(0.0), ao, additive, atten);
        ...
        vec3 irradianceL = vec3(0);
        vec3 glossenv = vec3(0);
        vec3 legacyenv = vec3(0);
        sampleReflectionProbesLegacy(irradianceL, glossenv, legacyenv, tc, pos.xyz, norm.xyz, spec.a, envIntensity, false, amblit_linear);
        adjustIrradiance(irradianceL, ambocc);

        // apply lambertian IBL only (see pbrIbl)
        color.rgb = irradianceL;
